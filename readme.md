...
The user jumps their way through the game, with the current cumulative value shown on the current (green block. The user can either single jump to add the value on the next tile to their current value, or double jump to skip the next tile, then add the one after that to the current value. The following images are how the instructions are shown in the game

| Game Mechanics | Single vs Double Jump | Current and Block Values | Game Layout |
|---|---|---|---|
